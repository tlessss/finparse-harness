...
```
你 →[钉钉]→ 机器人 →(Stream长连接)→ bridge.py ──喂消息──▶ 常驻 claude 会话
你 ←[钉钉]←─────── 回答 + 实时进度 ◀── (stream-json 进出) ─┘
                                          └──镜像打印到终端(你能旁观)
```

**常驻会话(单进程)**:bridge 启动一个长期存活的 `claude`(stream-json 输入/输出),
所有钉钉消息喂进同一个进程 → **跨消息累积上下文**(记得前面聊过什么),就像一直开着的
终端会话。每一步(工具调用/说明)实时推钉钉、同时打印到终端供旁观。`/reset` 重启进程、清空上下文。

> 单进程=单段上下文,所有白名单用户共享同一会话(个人自用最省事)。要按用户隔离可扩展。
