...
def report_table_doc(code: str, year: int, field: str = "revenue") -> str:
    """待解析报告"选中目标表"的去数字+标题文字骨架——供**表级向量路由**(匹认证解析器)。
    ⚠️ 路由 key = 选表**之后**那张锚确认的目标表的骨架:用 select_table(向量召回 + 锚精判 + 维度闸)
    选出真目标表,取它的骨架。**不再**用裸的 top-1 向量猜(那没过锚,可能选错)。取不到 → 空串。"""
    from src.eval.table_cache import get_tables
    tables = get_tables(code, year)
    if not tables:
        return ""
    pick = select_table(tables, code, year, field)
    if not pick or not pick.get("table"):
        return ""
    return _table_textdoc(pick.get("table"), pick.get("caption", ""))
